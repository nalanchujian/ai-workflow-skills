...
4. 生成严格结构化的工作单元声明。每个 `units` 项只使用 `id`、`title`、`goal`、`acceptanceRefs`、`factRefs`、`decisionRefs`、`steps`、`verification`、`blockedBy`、`dependsOn`。`verification` 只能引用 AIW 上下文中列出的项目测试能力，格式为 `{ profile: <能力 ID>, targets: [<可选测试目标>] }`；不得写命令字符串、自然语言或自行猜测测试工具。每个单元必须显式引用实施依据的事实和决策；`blockedBy` 中的决策也必须出现在 `decisionRefs`。
...
- 每个单元都有目标、AC、事实、决策、步骤和项目已提供的测试能力引用；依赖图无循环。
